# TODO: look in some qExactive data for the notch
# 
# Author: cshughes
###############################################################################
######################
...
infiles = dir("/Users/cshughes/Documents/projects/tmtPlexing/publishedData/kroksveen", pattern=".mzML", full.names=TRUE)
...
for (i in 1:10){
	...
	hdMS3 = hd[hd$msLevel ==2,]
	#print number of MS3 spectra
	message(paste('Number of MS2 spectra = ',nrow(hdMS3),sep=''))
	#quant the data
	...
	hdMS3 = subset(hdMS3, rowSums(is.na(hdMS3[,23:28]))<6)
...
###bind all of the raw files together
lsetout = data.frame()
for (x in 1:10){
...
#make the plot
pdf('ch_Notch_qExactiveHF_intensity-histogram.pdf')
...
####get the proportion
######################
nth126 = hist(log10(lsetout$x126),breaks=200,plot=FALSE)
